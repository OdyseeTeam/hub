...
from hub.db.prefixes import (
    ACTIVATED_CLAIM_TXO_TYPE,
    ACTIVATED_SUPPORT_TXO_TYPE,
    BidOrderKey,
    ClaimTakeoverValue,
    ClaimToTXOValue,
    DBStatePrefixRow,
    HashXMempoolStatusPrefixRow,
    MempoolTXPrefixRow,
    PendingActivationKey,
    PendingActivationValue,
    PrefixDB,
    TXOToClaimValue,
)
...
class SecondaryDB:
    ...
    def __init__(
        ...
        if secondary_name:
            assert max_open_files == -1, (
                "max open files must be -1 for secondary readers"
            )
        ...
        self.encoded_headers = LRUCacheWithMetrics(
            1024, metric_name="encoded_headers", namespace=NAMESPACE
        )
        ...
        self.merkle_cache = LFUCacheWithMetrics(
            merkle_cache_size, metric_name="merkle", namespace=NAMESPACE
        )
    ...
    def get_activation(self, tx_num, position, is_support=False) -> int:
        activation = self.prefix_db.activated.get(
            ACTIVATED_SUPPORT_TXO_TYPE if is_support else ACTIVATED_CLAIM_TXO_TYPE,
            tx_num,
            position,
        )
        if activation:
    ...
    def get_supported_claim_from_txo(
        self, tx_num: int, position: int
    ) -> typing.Tuple[Optional[bytes], Optional[int]]:
    ...
    def get_supports(self, claim_hash: bytes):
        return [
            (k.tx_num, k.position, v.amount)
            for k, v in self.prefix_db.claim_to_support.iterate(prefix=(claim_hash,))
        ]

    def get_short_claim_id_url(
        self,
        name: str,
        normalized_name: str,
        claim_hash: bytes,
        ...
        for prefix_len in range(10):
            for k in self.prefix_db.claim_short_id.iterate(
                prefix=(normalized_name, claim_id[: prefix_len + 1]),
                include_value=False,
            ):
    ...
    def _prepare_resolve_result(
        ...
        short_url = self.get_short_claim_id_url(
            name, normalized_name, claim_hash, root_tx_num, root_position
        )
        ...
        if channel_hash:
            ...
            if channel_vals:
                channel_short_url = self.get_short_claim_id_url(
                    channel_vals.name,
                    channel_vals.normalized_name,
                    channel_hash,
                    channel_vals.root_tx_num,
        ...
        return ResolveResult(
            ...
            claim_amount,
            short_url=short_url,
            is_controlling=controlling_claim.claim_hash == claim_hash,
            canonical_url=canonical_url,
            last_takeover_height=last_take_over_height,
            claims_in_channel=claims_in_channel,
            creation_height=created_height,
    ...
    async def _batch_resolve_parsed_urls(
        self, args: List[Tuple[str, Optional[str], Optional[int]]]
    ) -> List[Optional[bytes]]:
        ...
        def get_txo_for_partial_claim_id(normalized: str, claim_id: str):
            for key, claim_txo in self.prefix_db.claim_short_id.iterate(
                prefix=(normalized, claim_id[:10])
            ):
        ...
        def get_claim_by_amount(normalized: str, order: int):
            ...
            for _idx, (key, claim_val) in enumerate(
                self.prefix_db.bid_order.iterate(prefix=(normalized,))
            ):
                if order > _idx + 1:
                    continue
        ...
        for idx, (name, claim_id, amount_order) in enumerate(args):
            ...
            if claim_id:
                ...
                txo = await run_in_executor(
                    self._executor,
                    get_txo_for_partial_claim_id,
                    normalized_name,
                    claim_id,
                )
                if txo:
        ...
        if needed_full_claim_hashes:
            ...
            async for (
                partial_claim_txo_k,
                v,
            ) in self.prefix_db.txo_to_claim.multi_get_async_gen(
                self._executor, list(unique_full_claims.keys())
            ):
        ...
        needed_winning = list(
            set(
                normalized_name
                for normalized_name, _ in needed
                if normalized_name is not None
            )
        )
        winning_indexes = [
            idx for idx in range(len(needed)) if needed[idx][0] is not None
        ]
        controlling_claims = {
            name: takeover_v
            async for (
                name,
            ), takeover_v in self.prefix_db.claim_takeover.multi_get_async_gen(
        ...
        return [claim_hash for _, claim_hash in needed]

    def _resolve_claim_in_channel(
        self,
        channel_hash: bytes,
        normalized_name: str,
        stream_claim_id: Optional[str] = None,
    ):
        for key, stream in self.prefix_db.channel_to_claim.iterate(
            prefix=(channel_hash, normalized_name)
        ):
            if stream_claim_id is not None and not stream.claim_hash.hex().startswith(
                stream_claim_id
            ):
                continue
    ...
    async def batch_resolve_urls(
        self, urls: List[str]
    ) -> Dict[str, ExpandedResolveResult]:
        """
        ...
        """
        run_in_executor = asyncio.get_event_loop().run_in_executor
        pending_results: Dict[str, ExpandedResolveResult] = {}
        ...
        needed: List[
            Tuple[str, str, int]
        ] = []  # (name, partial claim id, amount order) of a url
        ...
        for url in urls:
            if pending_claims:
                pending_result = pending_claims.resolve(url)
                if pending_result is not None:
                    pending_results[url] = pending_result
                    continue
            need_args = None
            try:
        ...
        outer_layer_claims = {
            claim_hash: urls_to_parts_mapping[args]
            for args, claim_hash in zip(
        ...
        needed_claims = set(
            claim_hash
            for claim_hash in outer_layer_claims.keys()
        ...
        if needed_claims:
            ...
            txos = {
                ...
                async for (
                    claim_hash,
                ), txo in self.prefix_db.claim_to_txo.multi_get_async_gen(
                    self._executor,
                    [(claim_hash,) for claim_hash in claims_to_check_if_in_channel],
                )
            }
            needed_claims.update(
                {
                    ...
                    async for _, claim in self.prefix_db.claim_to_channel.multi_get_async_gen(
                        ...
                        [
                            (
                                claim_hash,
                                txos[claim_hash].tx_num,
                                txos[claim_hash].position,
                            )
                            for claim_hash in needed_claims
                            if txos[claim_hash] is not None
                        ],
                    )
        ...
        for url, streams in needed_streams_in_channels.items():
            ...
            for stream in streams:
                stream_claim_hash = await run_in_executor(
                    self._executor,
                    self._resolve_claim_in_channel,
                    resolved_channel_hash,
                    stream.normalized,
                    stream.claim_id,
                )
        ...
        results = {}
        for url in urls:
            if url in pending_results:
                results[url] = pending_results[url]
                continue
            claim_hash = resolved_urls.get(url)
            parsed = parsed_urls[url]
            if not claim_hash or not claims[claim_hash]:
                if (
                    not isinstance(parsed, Exception)
                    and parsed.has_channel
                    and not parsed.has_stream
                ):
                    results[url] = ExpandedResolveResult(
                        None,
                        LookupError(f'Could not find channel in "{url}".'),
                        None,
                        None,
                    )
                elif (
                    not isinstance(parsed, Exception)
                    and not parsed.has_channel
                    and parsed.has_stream
                ):
                    results[url] = ExpandedResolveResult(
                        LookupError(f'Could not find claim at "{url}".'),
                        None,
                        None,
                        None,
                    )
                    ...
                    results[url] = ExpandedResolveResult(parsed, None, None, None)
                continue
            ...
            if parsed.has_stream_in_channel or (
                not isinstance(claim, Exception) and claim.channel_hash
            ):
                stream = claim
            ...
            if claim and not isinstance(claim, Exception) and claim.reposted_claim_hash:
                if claim.reposted_claim_hash in claims:
                    ...
                    if (
                        repost
                        and not isinstance(repost, Exception)
                        and repost.channel_hash
                        and repost.channel_hash in claims
                    ):
                        reposted_channel = claims[repost.channel_hash]
            results[url] = ExpandedResolveResult(
                stream, channel, repost, reposted_channel
            )
        results.update(pending_results)
        return results
    ...
    def get_support_txo_amount(
        self, claim_hash: bytes, tx_num: int, position: int
    ) -> Optional[int]:
        ...
        if height >= self.db_height + 1:
            ...
            return amount + sum(
                ...
                for v in self.prefix_db.active_amount.iterate(
                    ...
                    stop=(claim_hash, txo_type, height),
                    include_key=False,
                )
            )
        return sum(
    ...
    def get_url_effective_amount(
        self, name: str, claim_hash: bytes
    ) -> Optional["BidOrderKey"]:
    ...
    def get_claims_for_name(self, name):
        ...
        stop = self.prefix_db.claim_short_id.pack_partial_key(name) + int(2).to_bytes(
            1, byteorder="big"
        )
        cf = self.prefix_db.column_families[self.prefix_db.claim_short_id.prefix]
        for _v in self.prefix_db.iterator(
            column_family=cf, start=prefix, iterate_upper_bound=stop, include_key=False
        ):
    ...
    async def _prepare_resolve_results(
        self,
        claim_hashes: List[bytes],
        ...
        reposts = {
            ...
            async for (
                claim_hash,
            ), repost in self.prefix_db.repost.multi_get_async_gen(
        ...
        claims = {
            ...
            async for (
                claim_hash,
            ), claim in self.prefix_db.claim_to_txo.multi_get_async_gen(
        ...
        channels = {
            ...
            async for (
                ...
                tx_num,
                position,
            ), signed in self.prefix_db.claim_to_channel.multi_get_async_gen(
                self._executor,
                [
                    (claim_hash, claims[claim_hash].tx_num, claims[claim_hash].position)
                    for claim_hash in claims_and_reposts
                    if claims[claim_hash] is not None
                ],
            )
            if signed
        }
        ...
        needed_channels = list(
            {
                channel_hash
                for channel_hash in channels.values()
                if channel_hash not in claims
            }
        )
        if needed_channels:
            claims.update(
                {
                    ...
                    async for (
                        ...
                        self._executor,
                        [(claim_hash,) for claim_hash in needed_channels],
                    )
                }
            )
        ...
        unique_names = list(
            sorted(
                {
                    claim_txo.normalized_name
                    for claim_txo in claims.values()
        ...
        claim_tx_hashes = {
            ...
            async for (tx_num,), tx_hash in self.prefix_db.tx_hash.multi_get_async_gen(
                self._executor,
                [(claim.tx_num,) for claim in claims.values() if claim is not None],
                False,
            )
        }
        ...
        supports = {
            ...
            async for (
                claim_hash,
            ), support in self.prefix_db.support_amount.multi_get_async_gen(
        ...
        claims_in_channels = {
            ...
            async for (
                claim_hash,
            ), v in self.prefix_db.channel_count.multi_get_async_gen(
                self._executor, [(claim_hash,) for claim_hash in claims]
            )
        }
        ...
        repost_counts = {
            ...
            async for (
                claim_hash,
            ), v in self.prefix_db.reposted_count.multi_get_async_gen(
                self._executor, [(claim_hash,) for claim_hash in claims]
            )
        }
        ...
        effective_amounts = {
            ...
            async for (
                claim_hash,
            ), v in self.prefix_db.effective_amount.multi_get_async_gen(
                self._executor, [(claim_hash,) for claim_hash in claims]
            )
        }
        ...
        def _get_censored_error(
            censor_type: str,
            canonical_url: str,
            channel_hash: bytes,
            repost_hash: bytes,
        ):
            channel = censoring_channels.get(
                channel_hash
            ) or self._fs_get_claim_by_hash(channel_hash)
            censoring_channels[channel_hash] = channel
            claim = censoring_reposts.get(repost_hash) or self._fs_get_claim_by_hash(
                repost_hash
            )
        ...
        def _prepare_result(touched, claim_txo):
            ...
            if apply_blocking:
                blocker = (
                    self.blocked_streams.get(touched)
                    or self.blocked_streams.get(reposted_claim_hash)
                    or self.blocked_channels.get(touched)
                    or self.blocked_channels.get(reposted_channel_hash)
                    or self.blocked_channels.get(channel_hash)
                )
                if blocker:
                    ...
                    return _get_censored_error(
                        "blocked",
                        canonical_url,
                        blocker_channel_hash,
                        blocker_repost_hash,
                    )

            if apply_filtering:
                filter_info = (
                    self.filtered_streams.get(touched)
                    or self.filtered_streams.get(reposted_claim_hash)
                    or self.filtered_channels.get(touched)
                    or self.filtered_channels.get(reposted_channel_hash)
                    or self.filtered_channels.get(channel_hash)
                )
                if filter_info:
            ...
            return ResolveResult(
                ...
                claim_amount,
                short_url=short_url,
                is_controlling=controlling_claim.claim_hash == touched,
                canonical_url=canonical_url,
                last_takeover_height=last_take_over_height,
                claims_in_channel=claims_in_channel,
                creation_height=created_height,
                ...
                if not channel_hash
                else claim_txo.channel_signature_is_valid,
                reposted_tx_hash=reposted_tx_hash,
                reposted_tx_position=reposted_tx_position,
                reposted_height=reposted_height,
                channel_tx_hash=channel_tx_hash,
        ...
        for touched in claim_hashes:
            ...
            if not claim_txo or (
                claim_txo and not controlling_claims[claim_txo.normalized_name]
            ):
                ...
                if claim.channel_hash:
                    ...
                    if channel_txo and not isinstance(channel_txo, Exception):
                        if claim.channel_hash in total_extra:
                            ...
                            extra[claim.channel_hash] = total_extra[
                                claim.channel_hash
                            ] = _prepare_result(claim.channel_hash, channel_txo)
                if claim.reposted_claim_hash:
                    ...
                    if repost_txo and not isinstance(repost_txo, Exception):
                        if claim.reposted_claim_hash in total_extra:
                            extra[claim.reposted_claim_hash] = total_extra[
                                claim.reposted_claim_hash
                            ]
                        else:
                            extra[claim.reposted_claim_hash] = total_extra[
                                claim.reposted_claim_hash
                            ] = _prepare_result(claim.reposted_claim_hash, repost_txo)
                        if (
                            not isinstance(claim, Exception)
                            and claim.reposted_claim_hash in channels
                        ):
                            reposted_channel_hash = channels[claim.reposted_claim_hash]
                            repost_channel_txo = claims[reposted_channel_hash]
                            if repost_channel_txo and not isinstance(
                                repost_channel_txo, Exception
                            ):
                                if reposted_channel_hash in total_extra:
                                    extra[reposted_channel_hash] = total_extra[
                                        reposted_channel_hash
                                    ]
                                else:
                                    extra[reposted_channel_hash] = total_extra[
                                        reposted_channel_hash
    ...
    def get_streams_and_channels_reposted_by_channel_hashes(
        self, reposter_channel_hashes: Set[bytes]
    ):
        streams, channels = {}, {}
        for reposter_channel_hash in reposter_channel_hashes:
            for stream in self.prefix_db.channel_to_claim.iterate(
                (reposter_channel_hash,), include_key=False
            ):
    ...
    def get_expired_by_height(
        self, height: int
    ) -> Dict[bytes, Tuple[int, int, str, TxInput]]:
        expired = {}
        for k, v in self.prefix_db.claim_expiration.iterate(prefix=(height,)):
            tx_hash = self.get_tx_hash(k.tx_num)
            tx = self.coin.transaction(
                self.prefix_db.tx.get(tx_hash, deserialize_value=False)
            )
            ...
            expired[v.claim_hash] = (
                ...
                TxInput(
                    prev_hash=tx_hash,
                    prev_idx=k.position,
                    script=tx.outputs[k.position].pk_script,
                    sequence=0,
                ),
            )
        return expired
    ...
    def get_claim_txos_for_name(self, name: str):
        txos = {}
        prefix = self.prefix_db.claim_short_id.pack_partial_key(name) + int(1).to_bytes(
            1, byteorder="big"
        )
        stop = self.prefix_db.claim_short_id.pack_partial_key(name) + int(2).to_bytes(
            1, byteorder="big"
        ...
        cf = self.prefix_db.column_families[self.prefix_db.claim_short_id.prefix]
        for v in self.prefix_db.iterator(
            column_family=cf, start=prefix, iterate_upper_bound=stop, include_key=False
        ):
    ...
    def get_claim_metadata(self, tx_hash, nout):
        ...
        try:
            ...
            self.logger.exception(
                "claim parsing for ES failed with tx: %s", tx_hash[::-1].hex()
            )
            return
    ...
    def get_activated_at_height(
        self, height: int
    ) -> DefaultDict[PendingActivationValue, List[PendingActivationKey]]:
        activated = defaultdict(list)
        for k, v in self.prefix_db.pending_activation.iterate(prefix=(height,)):
            activated[v].append(k)
        return activated

    def get_future_activated(
        self, height: int
    ) -> typing.Dict[PendingActivationValue, PendingActivationKey]:
        results = {}
        for k, v in self.prefix_db.pending_activation.iterate(
            start=(height + 1,),
            stop=(height + 1 + self.coin.maxTakeoverDelay,),
            reverse=True,
        ):
            if v not in results:
                results[v] = k
    ...
    async def _read_tx_counts(self):
        ...
        def get_counts():
            return [
                v.tx_count
                for v in self.prefix_db.tx_count.iterate(
                    start=(0,),
                    stop=(self.db_height + 1,),
                    include_key=False,
                    fill_cache=False,
                )
            ]

        tx_counts = await asyncio.get_event_loop().run_in_executor(
            self._executor, get_counts
        )
        assert len(tx_counts) == self.db_height + 1, (
            f"{len(tx_counts)} vs {self.db_height + 1}"
        )
    ...
    async def _read_block_hashes(self):
        def get_block_hashes():
            return [
                block_hash
                for block_hash in self.prefix_db.block_hash.iterate(
                    start=(0,),
                    stop=(self.db_height + 1,),
                    include_key=False,
                    fill_cache=False,
        ...
        block_hashes = await asyncio.get_event_loop().run_in_executor(
            self._executor, get_block_hashes
        )
    ...
    async def _read_tx_hashes(self):
        def _read_tx_hashes():
            return list(
                self.prefix_db.tx_hash.iterate(
                    start=(0,),
                    stop=(self.db_tx_count + 1,),
                    include_key=False,
                    fill_cache=False,
                    deserialize_value=False,
                )
        ...
        self.total_transactions.extend(
            await asyncio.get_event_loop().run_in_executor(
                self._executor, _read_tx_hashes
            )
        )
        self.tx_num_mapping = {
        ...
        self.logger.info(
            "loaded %i tx hashes in %ss", len(self.total_transactions), round(ts, 4)
        )
    ...
    def open_db(self):
        ...
        secondary_path = (
            ""
            if not self._secondary_name
            else os.path.join(self._db_dir, self._secondary_name)
        )
        open_db_canary = None

        if self._secondary_name:
            open_db_canary = os.path.join(
                self._db_dir, f"{self._secondary_name}-db-canary"
            )
            if os.path.exists(open_db_canary):
                with open(self._need_restart_path, "w+") as f:
                    f.write(
                        f"{time.strftime(f'%Y-%m-%d %H:%M:%S')} {self._secondary_name}\n"
                    )
            ...
            es_sync_db_canary = os.path.join(
                self._db_dir, "lbry-elastic-writer-db-canary"
            )
        ...
        self.prefix_db = PrefixDB(
            db_path,
            reorg_limit=self._reorg_limit,
            max_open_files=self._db_max_open_files,
            unsafe_prefixes={
                DBStatePrefixRow.prefix,
                MempoolTXPrefixRow.prefix,
                HashXMempoolStatusPrefixRow.prefix,
            },
            secondary_path=secondary_path,
            enforce_integrity=self._enforce_integrity,
        )
        ...
        if self.last_indexed_address_status_height:
            self.logger.info(
                f"last indexed address statuses at block {self.last_indexed_address_status_height}"
            )
        self.logger.info(f"using address status index: {self._index_address_status}")
        if self.filtering_channel_hashes:
            self.logger.info(
                "filtering claims reposted by channels: %s",
                ", ".join(map(lambda x: x.hex(), self.filtering_channel_hashes)),
            )
        if self.blocking_channel_hashes:
            self.logger.info(
                "blocking claims reposted by channels: %s",
    ...
    def _get_hashX_statuses(
        ...
        return [
            None if hashX not in statuses else statuses[hashX].hex()
    ...
    def _get_tx_hashes(self, tx_nums: List[int]) -> List[Optional[bytes]]:
        if self._cache_all_tx_hashes:
            return [
                None if tx_num > self.db_tx_count else self.total_transactions[tx_num]
                for tx_num in tx_nums
            ]
        return self.prefix_db.tx_hash.multi_get(
            [(tx_num,) for tx_num in tx_nums], deserialize_value=False
        )
        ...
        if self._cache_all_tx_hashes:
            ...
            for tx_num in tx_nums:
                append_result(
                    ...
                    else self.total_transactions[tx_num]
                )
    ...
    def get_tx_nums(self, tx_hashes: List[bytes]) -> Dict[bytes, Optional[int]]:
        ...
        return {
            k: None if not v else v.tx_num
            for k, v in zip(
                tx_hashes,
                self.prefix_db.tx_num.multi_get([(tx_hash,) for tx_hash in tx_hashes]),
            )
        }
    ...
    def encode_headers(self, start_height, count, headers):
        ...
        if not self.encoded_headers.get(key):
            ...
            headers = base64.b64encode(
                compressobj.compress(headers) + compressobj.flush()
            ).decode()
    ...
    async def read_headers(self, start_height, count) -> typing.Tuple[bytes, int]:
        ...
        if start_height < 0 or count < 0:
            raise DBError(
                f"{count:,d} headers starting at {start_height:,d} not on disk"
            )
    ...
    async def get_transactions_and_merkles(self, txids: List[str]):
        ...
        for txid in txids:
            ...
            if cached_tx:
                ...
                if tx_height > 0:
                    ...
                    tx_heights_and_positions[tx_height].append(
                        (tx_hash_bytes, tx, tx_num, tx_pos)
                    )
                else:
                    cached_mempool.append((tx_hash_bytes, tx))
            else:
                if self._cache_all_tx_hashes and tx_hash_bytes in self.tx_num_mapping:
                    needed_confirmed.append(
                        (tx_hash_bytes, self.tx_num_mapping[tx_hash_bytes])
                    )
                else:
                    needed_tx_nums.append(tx_hash_bytes)

        if needed_tx_nums:
            for tx_hash_bytes, v in zip(
                needed_tx_nums,
                await run_in_executor(
                    self._executor,
                    self.prefix_db.tx_num.multi_get,
                    [(tx_hash,) for tx_hash in needed_tx_nums],
                    True,
                    True,
                ),
            ):
        ...
        if needed_confirmed:
            for (tx_hash_bytes, tx_num), tx in zip(
                needed_confirmed,
                await run_in_executor(
                    self._executor,
                    self.prefix_db.tx.multi_get,
                    [(tx_hash,) for tx_hash, _ in needed_confirmed],
                    True,
                    False,
                ),
            ):
        ...
        merkles: Dict[
            int, FastMerkleCacheItem
        ] = {}  # uses existing cached merkle trees when they're available
        ...
        if needed_mempool:
            ...
            for tx_hash_bytes, tx in zip(
                ...
                await asyncio.sleep(0)
        return {
            txid: tx_infos.get(txid) for txid in txids
        }  # match ordering of the txs in the request

    async def fs_block_hashes(self, height, count):
        if height + count > self.db_height + 1:
            raise DBError(
                f"only got {len(self.block_hashes) - height:,d} headers starting at {height:,d}, not {count:,d}"
            )
    ...
    def _read_history(self, hashX: bytes, limit: Optional[int] = 1000) -> List[int]:
        ...
        for hist in self.prefix_db.hashX_history.iterate(
            prefix=(hashX,), include_key=False
        ):
            txs_extend(hist)
    ...
    async def read_history(
        self, hashX: bytes, limit: Optional[int] = 1000
    ) -> List[int]:
        return await asyncio.get_event_loop().run_in_executor(
            self._executor, self._read_history, hashX, limit
        )
    ...
    def read_db_state(self):
        ...
        if not state:
            ...
            if self.db_version not in self.DB_VERSIONS:
                raise DBError(
                    f"your DB version is {self.db_version} but this "
            ...
            self.last_indexed_address_status_height = (
                state.hashX_status_last_indexed_height
            )
        return state
    ...
    def assert_db_state(self):
        ...
        assert self.db_version == state.db_version, (
            f"{self.db_version} != {state.db_version}"
        )
        assert self.db_height == state.height, f"{self.db_height} != {state.height}"
        assert self.db_tx_count == state.tx_count, (
            f"{self.db_tx_count} != {state.tx_count}"
        )
        assert self.db_tip == state.tip, f"{self.db_tip} != {state.tip}"
        assert self.catching_up == state.catching_up, (
            f"{self.catching_up} != {state.catching_up}"
        )
        assert self.es_sync_height == state.es_sync_height, (
            f"{self.es_sync_height} != {state.es_sync_height}"
        )
    ...
    async def all_utxos(self, hashX):
        ...
        def read_utxos():
            ...
            utxo_info = [
                (k.tx_num, k.nout, v.amount)
                for k, v in self.prefix_db.utxo.iterate(prefix=(hashX,))
            ]
            return [
                UTXO(tx_num, nout, *fs_tx_hash(tx_num), value=value)
                for (tx_num, nout, value) in utxo_info
            ]

        while True:
            utxos = await asyncio.get_event_loop().run_in_executor(
                self._executor, read_utxos
            )
            if all(utxo.tx_hash is not None for utxo in utxos):
                return utxos
            self.logger.warning(f"all_utxos: tx hash not found (reorg?), retrying...")
