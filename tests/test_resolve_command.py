...
class BaseResolveTestCase(CommandTestCase):
    def assertMatchESClaim(self, claim_from_es, claim_from_db):
        self.assertEqual(
            claim_from_es["claim_hash"][::-1].hex(), claim_from_db.claim_hash.hex()
        )
        self.assertEqual(claim_from_es["claim_id"], claim_from_db.claim_hash.hex())
        self.assertEqual(
            claim_from_es["activation_height"],
            claim_from_db.activation_height,
            f"es height: {claim_from_es['activation_height']}, rocksdb height: {claim_from_db.activation_height}",
        )
        self.assertEqual(
            claim_from_es["last_take_over_height"], claim_from_db.last_takeover_height
        )
        ...
        self.assertEqual(
            claim_from_es["effective_amount"], claim_from_db.effective_amount
        )
    ...
    async def assertNoClaimForName(self, name: str):
        lbrycrd_winning = json.loads(
            await self.blockchain._cli_cmnd("getclaimsforname", name)
        )
    ...
    async def assertNoClaim(self, name: str, claim_id: str):
        expected = json.loads(
            await self.blockchain._cli_cmnd(
                "getclaimsfornamebyid", name, '["' + claim_id + '"]'
            )
        )
    ...
    async def assertMatchWinningClaim(self, name):
        expected = json.loads(
            await self.blockchain._cli_cmnd("getclaimsfornamebybid", name, "[0]")
        )
        stream, channel, _, _ = await self.conductor.spv_node.server.db.resolve(name)
    ...
    async def assertMatchClaim(self, name, claim_id, is_active_in_lbrycrd=True):
        ...
        self.assertEqual(len(claim_from_es[0]), 1)
        self.assertEqual(
            claim_from_es[0][0]["claim_hash"][::-1].hex(), claim.claim_hash.hex()
        )
        self.assertMatchESClaim(claim_from_es[0][0], claim)
        ...
        if is_active_in_lbrycrd:
            ...
            self._check_supports(
                claim.claim_hash.hex(),
                expected["claims"][0].get("supports", []),
                claim_from_es[0][0]["support_amount"],
            )
        else:
            if "claims" in expected and expected["claims"] is not None:
    ...
    async def assertMatchClaimIsWinning(self, name, claim_id):
        self.assertEqual(
            claim_id, (await self.assertMatchWinningClaim(name)).claim_hash.hex()
        )
    ...
    def _check_supports(self, claim_id, lbrycrd_supports, es_support_amount):
        ...
        for i, (tx_num, position, amount) in enumerate(es_supports):
            ...
            support = next(
                filter(
                    lambda s: s["txid"] == txid and s["n"] == position, lbrycrd_supports
                )
            )
    ...
    async def assertMatchClaimsForName(self, name):
        expected = json.loads(
            await self.blockchain._cli_cmnd("getclaimsforname", name, "", "true")
        )
        db = self.conductor.spv_node.server.db
        ...
        for c in expected["claims"]:
            ...
            self.assertMatchESClaim(claim_from_es[0][0], claim)
            self._check_supports(
                claim_id, c.get("supports", []), claim_from_es[0][0]["support_amount"]
            )

    async def assertNameState(
        self,
        height: int,
        name: str,
        winning_claim_id: str,
        last_takeover_height: int,
        ...
        for non_winning in non_winning_claims:
            claim = await self.assertMatchClaim(
                name,
                non_winning.claim_id,
                is_active_in_lbrycrd=non_winning.active_in_lbrycrd,
            )
...
class ResolveCommand(BaseResolveTestCase):
    async def test_colliding_short_id(self):
        ...
        for c1, c2 in zip(colliding_claim_ids[0], colliding_claim_ids[1]):
            if c1 == c2:
                ...
                break
        await self.assertResolvesToClaimId(
            f"@abc#{colliding_claim_ids[0][:2]}", colliding_claim_ids[0]
        )
        await self.assertResolvesToClaimId(
            f"@abc#{colliding_claim_ids[0][:7]}", colliding_claim_ids[0]
        )
        await self.assertResolvesToClaimId(
            f"@abc#{colliding_claim_ids[0][:17]}", colliding_claim_ids[0]
        )
        await self.assertResolvesToClaimId(
            f"@abc#{colliding_claim_ids[0]}", colliding_claim_ids[0]
        )
        await self.assertResolvesToClaimId(
            f"@abc#{colliding_claim_ids[1][: collision_depth + 1]}",
            colliding_claim_ids[1],
        )
        await self.assertResolvesToClaimId(
            f"@abc#{colliding_claim_ids[1][:7]}", colliding_claim_ids[1]
        )
        await self.assertResolvesToClaimId(
            f"@abc#{colliding_claim_ids[1][:17]}", colliding_claim_ids[1]
        )
        await self.assertResolvesToClaimId(
            f"@abc#{colliding_claim_ids[1]}", colliding_claim_ids[1]
        )
    ...
    async def test_abandon_channel_and_claims_in_same_tx(self):
        ...
        foo2_id = self.get_claim_id(
            await self.stream_create("foo2", "0.01", channel_id=channel_id)
        )
        await self.stream_update(
            foo2_id, bid="0.0001", channel_id=channel_id, confirm=False
        )
        tx = await self.stream_create(
            "foo3", "0.01", channel_id=channel_id, confirm=False, return_tx=True
        )
    ...
    async def test_resolve_response(self):
        ...
        self.assertEqual(
            claim["timestamp"], self.ledger.headers.estimated_timestamp(claim["height"])
        )
        self.assertEqual(
        ...
        response = await self.out(
            self.daemon.jsonrpc_resolve(["lbry://foo", "lbry://foo2"])
        )
        ...
        self.assertEqual(
            claim["confirmations"], json.loads(tx_details)["confirmations"]
        )
    ...
    async def test_abandoned_channel_with_signed_claims(self):
        ...
        orphan_claim = await self.stream_create(
            "on-channel-claim", "0.0001", channel_id=channel["claim_id"]
        )
        ...
        self.assertEqual(
            {"channel_id": abandoned_channel_id}, response["signing_channel"]
        )
        ...
        valid_claim = await self.stream_create(
            "on-channel-claim", "0.00000001", channel_id=channel["claim_id"]
        )
        # resolves normally
        ...
        claims = [
            await self.resolve("on-channel-claim"),
            await self.resolve("on-channel-claim$2"),
        ]
        self.assertEqual(2, len(claims))
        self.assertEqual(
            {channel["claim_id"]},
            {claim["signing_channel"]["claim_id"] for claim in claims},
        )

    async def test_normalization_resolution(self):

        one = "Î£Î¯ÏƒÏ…Ï†Î¿Ï‚ï¬Ã†"
        two = "Î£ÎŠÎ£Î¥Î¦ÎŸÏƒFIÃ¦"
    ...
    async def test_resolve_old_claim(self):
        channel = await self.daemon.jsonrpc_channel_create(
            "@olds", "1.0", blocking=True
        )
        ...
        claim.publisherSignature.signature = bytes(
            reversed(claim.publisherSignature.signature)
        )
        tx = await Transaction.claim_create(
            "bad_example",
            claim.SerializeToString(),
    ...
    async def test_resolve_with_includes(self):
        wallet2 = await self.daemon.jsonrpc_wallet_create(
            "wallet2", create_account=True
        )
        ...
        resolve = await self.resolve(
            "priced", wallet_id=wallet2.id, include_is_my_output=True
        )
...
class ResolveClaimTakeovers(BaseResolveTestCase):
    async def test_channel_invalidation(self):
        channel_id = (await self.channel_create("@test", "0.1"))["outputs"][0][
            "claim_id"
        ]
        channel_id2 = (await self.channel_create("@other", "0.1"))["outputs"][0][
            "claim_id"
        ]

        async def make_claim(name, amount, channel_id=None):
            return (await self.stream_create(name, amount, channel_id=channel_id))[
                "outputs"
            ][0]["claim_id"]

        unsigned_then_signed = await make_claim("unsigned_then_signed", "0.1")
        unsigned_then_updated_then_signed = await make_claim(
            "unsigned_then_updated_then_signed", "0.1"
        )
        ...
        await self.assertMatchClaimIsWinning(
            "unsigned_then_signed", unsigned_then_signed
        )
        self.assertIn(
            "error", await self.resolve("@test/unsigned_then_updated_then_signed")
        )
        await self.assertMatchClaimIsWinning(
            "unsigned_then_updated_then_signed", unsigned_then_updated_then_signed
        )
        self.assertDictEqual(
            await self.resolve("@test/signed_then_unsigned"),
            await self.resolve("signed_then_unsigned"),
        )
        await self.assertMatchClaimIsWinning(
            "signed_then_unsigned", signed_then_unsigned
        )
        # sign 'unsigned_then_signed' and update it
        await self.ledger.wait(
            await self.daemon.jsonrpc_stream_update(
        ...
        await self.ledger.wait(
            await self.daemon.jsonrpc_stream_update(
                unsigned_then_updated_then_signed, "0.09"
            )
        )
        await self.ledger.wait(
            await self.daemon.jsonrpc_stream_update(
        ...
        await self.assertMatchClaimIsWinning(
            "signed_then_signed_different_chan", signed_then_signed_different_chan
        )
        await self.assertMatchClaimIsWinning("signed3", signed3)
        await self.assertMatchClaimIsWinning("signed4", signed4)

        self.assertDictEqual(
            await self.resolve("@other/signed_then_signed_different_chan"),
            await self.resolve("signed_then_signed_different_chan"),
        )
        self.assertDictEqual(
            await self.resolve("@other/signed4"), await self.resolve("signed4")
        )
    ...
    async def _test_activation_delay(self):
        ...
        first_claim_id = (
            await self.stream_create(name, "0.1", allow_duplicate_name=True)
        )["outputs"][0]["claim_id"]
        ...
        second_claim_id = (
            await self.stream_create(name, "0.2", allow_duplicate_name=True)
        )["outputs"][0]["claim_id"]
        # sanity check
    ...
    async def create_stream_claim(self, amount: str, name="derp") -> str:
        return (await self.stream_create(name, amount, allow_duplicate_name=True))[
            "outputs"
        ][0]["claim_id"]
    ...
    async def test_delay_takeover_with_update(self):
        ...
        await self.assertNameState(
            height=537,
            name=name,
            winning_claim_id=first_claim_id,
            last_takeover_height=207,
            non_winning_claims=[
                ClaimStateValue(
                    second_claim_id, activation_height=538, active_in_lbrycrd=False
                ),
                ClaimStateValue(
                    third_claim_id, activation_height=539, active_in_lbrycrd=False
                ),
            ],
        )
        ...
        await self.assertNameState(
            ...
            non_winning_claims=[
                ClaimStateValue(
                    second_claim_id, activation_height=538, active_in_lbrycrd=True
                ),
                ClaimStateValue(
                    third_claim_id, activation_height=539, active_in_lbrycrd=False
                ),
        ...
        await self.assertNameState(
            ...
            non_winning_claims=[
                ...
                ClaimStateValue(
                    third_claim_id, activation_height=539, active_in_lbrycrd=True
                ),
            ],
        )
        ...
        await self.assertNameState(
            ...
            non_winning_claims=[
                ...
                ClaimStateValue(
                    third_claim_id, activation_height=550, active_in_lbrycrd=False
                ),
            ],
        )
        ...
        await self.assertNameState(
            height=550,
            name=name,
            winning_claim_id=third_claim_id,
            last_takeover_height=550,
            non_winning_claims=[
                ClaimStateValue(
                    first_claim_id, activation_height=207, active_in_lbrycrd=True
                ),
                ClaimStateValue(
                    second_claim_id, activation_height=538, active_in_lbrycrd=True
                ),
    ...
    async def test_delay_takeover_with_update_then_update_to_lower_before_takeover(
        self,
    ):
        name = "derp"
        first_claim_id = await self.create_stream_claim("0.2", name)
        ...
        await self.assertNameState(
            ...
            non_winning_claims=[
                ...
                ClaimStateValue(
                    third_claim_id, activation_height=559, active_in_lbrycrd=False
                ),
            ],
        )
        ...
        await self.assertNameState(
            ...
            non_winning_claims=[
                ...
                ClaimStateValue(
                    third_claim_id, activation_height=559, active_in_lbrycrd=True
                ),
            ],
        )
    ...
    async def test_delay_takeover_with_update_then_update_to_lower_on_takeover(self):
        ...
        await self.assertNameState(
            ...
            non_winning_claims=[
                ...
                ClaimStateValue(
                    third_claim_id, activation_height=560, active_in_lbrycrd=False
                ),
            ],
        )
        await self.generate(10)
        await self.assertNameState(
            ...
            non_winning_claims=[
                ...
                ClaimStateValue(
                    third_claim_id, activation_height=560, active_in_lbrycrd=True
                ),
            ],
        )
    ...
    async def test_delay_takeover_with_update_then_update_to_lower_after_takeover(self):
        ...
        await self.assertNameState(
            ...
            name=name,
            winning_claim_id=first_claim_id,
            last_takeover_height=551,
            non_winning_claims=[
                ClaimStateValue(
                    second_claim_id, activation_height=538, active_in_lbrycrd=True
                ),
                ClaimStateValue(
                    third_claim_id, activation_height=551, active_in_lbrycrd=True
                ),
            ],
        )
    ...
    async def test_resolve_signed_claims_with_fees(self):
        ...
        self.assertEqual(
            channel_id,
            (await self.assertMatchWinningClaim(channel_name)).claim_hash.hex(),
        )
        stream_name = "foo"
        ...
        self.assertSetEqual(
            set(greater_than_or_equal_to_zero), {stream_with_no_fee, stream_with_fee}
        )
        greater_than_zero = [
            claim["claim_id"]
            for claim in (
    ...
    async def test_spec_example(self):
        ...
        claim_id_A = (
            await self.stream_create(name, "10.0", allow_duplicate_name=True)
        )["outputs"][0]["claim_id"]
        ...
        claim_id_B = (
            await self.stream_create(name, "20.0", allow_duplicate_name=True)
        )["outputs"][0]["claim_id"]
        claim_B, _, _, _ = await self.conductor.spv_node.server.db.resolve(
            f"{name}:{claim_id_B}"
        )
        ...
        claim_id_C = (
            await self.stream_create(name, "50.0", allow_duplicate_name=True)
        )["outputs"][0]["claim_id"]
        self.assertEqual(1123, self.conductor.spv_node.server.db.db_height)
        claim_C, _, _, _ = await self.conductor.spv_node.server.db.resolve(
            f"{name}:{claim_id_C}"
        )
        ...
        claim_id_D = (
            await self.stream_create(name, "300.0", allow_duplicate_name=True)
        )["outputs"][0]["claim_id"]
        self.assertEqual(1132, self.conductor.spv_node.server.db.db_height)
        claim_D, _, _, _ = await self.conductor.spv_node.server.db.resolve(
            f"{name}:{claim_id_D}"
        )
    ...
    async def test_early_takeover(self):
        ...
        third_claim_id = (
            await self.stream_create(name, "0.3", allow_duplicate_name=True)
        )["outputs"][0]["claim_id"]
        self.assertNotEqual(first_claim_id, second_claim_id)
        # takeover should not have happened yet
        await self.assertMatchClaimIsWinning(name, first_claim_id)
    ...
    async def test_early_takeover_from_support_zero_delay(self):
        ...
        await self.assertMatchClaimIsWinning(name, first_claim_id)
        third_claim_id = (
            await self.stream_create(name, "0.19", allow_duplicate_name=True)
        )["outputs"][0]["claim_id"]
        await self.assertMatchClaimIsWinning(name, first_claim_id)
    ...
    async def test_early_takeover_from_support_and_claim_zero_delay(self):
        ...
        tx = await self.daemon.jsonrpc_stream_create(
            name, "0.19", file_path=file_path, allow_duplicate_name=True
        )
        await self.ledger.wait(tx)
    ...
    async def test_block_takeover_with_delay_1_support(self):
        name = "derp"
        # initially claim the name
        first_claim_id = (await self.stream_create(name, "0.1"))["outputs"][0][
            "claim_id"
        ]
        self.assertEqual(
            first_claim_id, (await self.assertMatchWinningClaim(name)).claim_hash.hex()
        )
        await self.generate(320)
        # a claim of higher amount made now will have a takeover delay of 10
        second_claim_id = (
    ...
    async def test_almost_prevent_takeover_remove_support_same_block_supported(self):
        name = "derp"
        first_claim_id, second_claim_id, tx = await self._test_almost_prevent_takeover(
            name, 9
        )
        await self.daemon.jsonrpc_txo_spend(type="support", txid=tx.id)
        await self.generate(1)
    ...
    async def test_almost_prevent_takeover_remove_support_one_block_after_supported(
        self,
    ):
        name = "derp"
        first_claim_id, second_claim_id, tx = await self._test_almost_prevent_takeover(
    ...
    async def test_abandon_before_takeover_no_delay_update(
        self,
    ):  # TODO: fix race condition line 506
        name = "derp"
        # initially claim the name
        first_claim_id = (await self.stream_create(name, "0.1"))["outputs"][0][
            "claim_id"
    ...
    async def test_abandon_controlling_support_before_pending_takeover(self):
        ...
        controlling_support_tx = await self.daemon.jsonrpc_support_create(
            first_claim_id, "0.9"
        )
        ...
        second_claim_id = (
            await self.stream_create(name, "0.9", allow_duplicate_name=True)
        )["outputs"][0]["claim_id"]
        ...
        tx = await self.daemon.jsonrpc_txo_spend(
            type="support", txid=controlling_support_tx.id, blocking=True
        )
        await self.generate(1)
        await self.assertNameState(
            538,
            ...
            non_winning_claims=[
                ClaimStateValue(
                    second_claim_id, activation_height=539, active_in_lbrycrd=False
                )
            ],
        )
        await self.generate(1)
    ...
    async def test_remove_controlling_support(self):
        name = "derp"
        # initially claim the name
        first_claim_id = (await self.stream_create(name, "0.2"))["outputs"][0][
            "claim_id"
        ]
        first_support_tx = await self.daemon.jsonrpc_support_create(
            first_claim_id, "0.9"
        )
        ...
        await self.generate(
            320
        )  # give the first claim long enough for a 10 block takeover delay
        await self.assertNameState(
            527, name, first_claim_id, last_takeover_height=207, non_winning_claims=[]
        )
        ...
        second_claim_support_tx = await self.daemon.jsonrpc_support_create(
            second_claim_id, "1.5"
        )
        await self.ledger.wait(second_claim_support_tx)
        await self.generate(
            1
        )  # neither the second claim or its support have activated yet
        await self.assertNameState(
            529,
            name,
            first_claim_id,
        ...
        await self.daemon.jsonrpc_txo_spend(
            type="support", claim_id=second_claim_id, blocking=True
        )
        await self.generate(1)  # support activates, takeover happens
        await self.assertNameState(
            540,
    ...
    async def test_claim_expiration(self):
        ...
        vanishing_claim = (await self.stream_create("vanish", "0.1"))["outputs"][0][
            "claim_id"
        ]

        await self.generate(493)
        # in block 701 and 702
        first_claim_id = (await self.stream_create(name, "0.3"))["outputs"][0][
            "claim_id"
        ]
    ...
    async def test_abandon_controlling_same_block_as_new_claim(self):
        ...
        await self.assertNameState(
            271, name, first_claim_id, last_takeover_height=207, non_winning_claims=[]
        )
        ...
        await self.assertNameState(
            272, name, second_claim_id, last_takeover_height=272, non_winning_claims=[]
        )
...
class ResolveAfterReorg(BaseResolveTestCase):
    async def reorg(self, start):
        ...
        # go back to start
        await self.blockchain.invalidate_block(
            (await self.ledger.headers.hash(start)).decode()
        )
        # go to previous + 1
    ...
    async def assertBlockHash(self, height):
        ...
        self.assertEqual(
            block_hash, (await reader_db.fs_block_hashes(height, 1))[0][::-1].hex()
        )
        txids = [tx_hash[::-1].hex() for tx_hash in reader_db.get_block_txs(height)]
        txs = await reader_db.get_transactions_and_merkles(txids)
        block_txs = (
            await self.conductor.spv_node.server.daemon.deserialised_block(block_hash)
        )["tx"]
        self.assertSetEqual(
            set(block_txs),
            set(txs.keys()),
            msg="leveldb/lbrycrd is missing transactions",
        )
        self.assertListEqual(
            block_txs, list(txs.keys()), msg="leveldb/lbrycrd transactions are of order"
        )
    ...
    async def test_reorg(self):
        ...
        await self.assertNameState(
            height=209,
            name=stream_name,
            winning_claim_id=stream_id,
            last_takeover_height=209,
            non_winning_claims=[],
        )
        await self.reorg(206)
        await self.assertNameState(
    ...
    async def test_reorg_change_claim_height(self):
        ...
        result = await self.resolve(
            "hovercraft"
        )  # TODO: do these for claim_search and resolve both
        ...
        self.assertDictEqual(
            {
                "error": {
                    "name": "NOT_FOUND",
                    "text": 'Could not find claim at "hovercraft".',
                }
            },
        ...
        new_txid = await self.blockchain.sendrawtransaction(
            hexlify(broadcast_tx.raw).decode()
        )
    ...
    async def test_pending_claim_resolve_and_search(self):
        name = "mempool-hovercraft"
        ...
        tx = await self.daemon.jsonrpc_stream_create(
            name, "1.0", file_path=self.create_upload_file(data=b"hi!")
        )
        await self.ledger.wait(tx)
    ...
    async def test_pending_signed_claim_resolve_and_search(self):
        ...
        resolved = await self.resolve("@mempool/signed-mempool")
        self.assertEqual(tx.id, resolved["txid"])
        self.assertEqual(0, resolved["height"])
        self.assertEqual("@mempool", resolved["signing_channel"]["name"])

        search_results = await self.claim_search(
            name="signed-mempool", channel="@mempool"
        )
        self.assertEqual(1, len(search_results))
        self.assertEqual(tx.id, search_results[0]["txid"])
        self.assertEqual(0, search_results[0]["height"])
...
def generate_signed_legacy(address: bytes, output: Output):
    ...
    digest = sha256(
        b"".join([decoded_address, claim.SerializeToString(), output.claim_hash[::-1]])
    )
